...
model['W0'] = np.random.randn(D,D) / np.sqrt(D)
...
def policy_forward0(x):
  """This is a manual implementation of a forward prop with an additional layer."""
  h0 = np.dot(model['W0'], x) # (H x H) . (H x D) = (H x D) (200 x 1)
  h0[h0<0] = 0
  h = np.dot(model['W1'], h0) # (H x D) . (D x 1) = (H x 1) (200 x 1)
  h[h<0] = 0 # ReLU introduces non-linearity
  logp = np.dot(model['W2'], h) # This is a logits function and outputs a decimal.   (1 x H) . (H x 1) = 1 (scalar)
  p = sigmoid(logp)  # squashes output to  between 0 & 1 range
  return p, h, h0 # return probability of taking action 2 (UP), and hidden state

def policy_backward0(eph0, eph, epx, epdlogp):
  """ backward pass. (eph is array of intermediate hidden states) """
  """ Manual implementation of a backward prop"""
  """ It takes an array of the hidden states that corresponds to all the images that were
  fed to the NN (for the entire episode, so a bunch of games) and their corresponding logp"""
...
#  dh = np.outer(epdlogp, model['W2'])
#  dh[eph <= 0] = 0 # backpro prelu
  W2W1 = np.dot(model['W2'],model['W1'])
  dh0 = np.outer(epdlogp, W2W1)
  dh0[eph0 <= 0] = 0 # backpro prelu 
  dW0 = np.dot(dh0.T, epx)
#  print (dW0.shape)
#  print (dW0.sum(axis=0))
  return {'W1':0, 'W2':0, 'W0':dW0}
...
W0list=[]
W0list.append(model['W0'])
env = wrappers.Monitor(env, 'tmp/pong-base', force=True, render_mode='mirror') # record the game as as an mp4 file
env._set_obs_type('mirror')

observation = env.reset()
prev_x = None # used in computing the difference frame
xs,hs,dlogps,drs, h0s = [],[],[],[],[]
running_reward = None
reward_sum = 0
episode_number = 0
while True:
  ...
  if done: # an episode finished
    ...
    epr = np.vstack(drs)
    xs,hs,dlogps,drs, h0s = [],[],[],[],[] # reset array memory
    ...
    grad = policy_backward0(eph0, eph, epx, epdlogp)
    ...
    if episode_number % 100 == 0: pickle.dump(model, open('save.px', 'wb'))
    reward_sum = 0
